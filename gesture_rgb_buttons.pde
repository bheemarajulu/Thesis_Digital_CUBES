...
const int  buttonPin_green_rst = 6;    // the pin that the pushbutton is attached to
...
void sensor_data()
{
 ...
 if (Serial.read() == 'M') //RGB Sensor Data
    {  
      altSerial.begin(38400); 
      //altSerial.println("A");
      //send_command();
 ...
 if (Serial.available()) {
   if (Serial.read() == 'A')// CUBE A Transmit 
    { 
     Serial.flush();
